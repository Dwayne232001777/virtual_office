...
function useVirtualizer_unstable(props) {
  ...
  this helps us skip re-calculations unless children/size changes */ const childProgressiveSizes = _react.useRef(new Array(getItemSize ? numItems : 0));
    if (virtualizerContext == null ? void 0 : virtualizerContext.childProgressiveSizes) {
        virtualizerContext.childProgressiveSizes.current = childProgressiveSizes.current;
    }
    const populateSizeArrays = _react.useCallback(()=>{
        if (!getItemSize) {
    ...
    const renderChildRows = _react.useCallback((_newIndex)=>{
        if (numItems === 0) {
    ...
    const updateChildArray = (_isScrolling)=>{
        // Render child changed, regenerate the child array
        const startIndex = Math.max(actualIndex, 0);
        const arrayLength = Math.min(virtualizerLength, Math.max(0, numItems - startIndex));
        const newChildArray = new Array(arrayLength);
        for(let i = 0; i < arrayLength; i++){
    ...
    const { setObserverList } = (0, _useIntersectionObserver.useIntersectionObserver)(_react.useCallback(// TODO: exclude types from this lint rule: https://github.com/microsoft/fluentui/issues/31286
    (entries, // eslint-disable-next-line no-restricted-globals, @typescript-eslint/no-unused-vars
    _observer)=>{
        ...
        const calculateOverBuffer = ()=>{
           ...
           */ let measurementPos = 0;
            if (latestEntry.target === afterElementRef.current) {
                ...
                measurementPos -= (_containerSizeRef_current = containerSizeRef.current) != null ? _containerSizeRef_current : 0;
        ...
        updateScrollPosition == null ? void 0 : updateScrollPosition(measurementPos);
        const maxIndex = Math.max(numItems + bufferItems - virtualizerLength, 0);
        ...
        (0, _reactdom.flushSync)(()=>{
            ...
            if (newStartIndex + virtualizerLength >= numItems && actualIndex + virtualizerLength >= numItems && !numItemsChanged) {
        ...
        root: scrollViewRef ? scrollViewRef == null ? void 0 : scrollViewRef.current : null,
        rootMargin: '0px',
        threshold: 0
    });
